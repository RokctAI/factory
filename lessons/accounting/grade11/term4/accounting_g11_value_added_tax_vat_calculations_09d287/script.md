...
Let's look at an example. Imagine a big bag of maize meal is marked at R2 875 on the shelf. This price *includes* VAT at 15%. So the R2 875 is our inclusive amount. The customer gives me R2 875, and I put it in my tin. But how much of that is mine, and how much belongs to SARS?
...
To find the VAT amount, we take the amount we *want* (the 15%), and divide it by the amount we *have* (the 115%).

So, our formula is:
Amount given multiplied by (what we want / what we have).
We have R2 875. We multiply it by 15 over 115.
R2 875 × (15 / 115).

Get your calculator. 2875 times 15, divided by 115. That gives us R375.
So, the VAT amount is R375. That's the money that goes to SARS.

Now, what is the price *excluding* VAT? This is easy. It's the total money in the tin, minus the tax man's share.
R2 875 minus R375.
That leaves R2 500.
So, the price excluding VAT is R2 500. That's my sales amount!
...
But where did this money come from? We have to record the credit entries.
Remember, we calculated that R2 500 is our actual sales income. So, we will **credit** the **Sales account** with R2 500. Why? Because Sales is an income, and income increases on the credit side, increasing owner's equity.

What about the R375? That is money we owe to SARS. It's a liability. We record this in an account called VAT Control. We will **credit** the **VAT Control account** with R375. Why? Because we owe this money to SARS, so our liability increases, and liabilities increase on the credit side.

Let's check our double-entry logic:
Debit Bank: R2 875.
Credit Sales: R2 500.
Credit VAT Control: R375.
Does our debit equal our credits? R2 500 + R375 = R2 875. Yes! The books balance perfectly.
