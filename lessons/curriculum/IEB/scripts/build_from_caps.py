...
set their own). So three IEB layers are *derived*, not hand-authored:
  ...
  {subject}/past_papers/index.json        IEB portal/library index scaffold
                                          (sessions[] filled by
                                          fetch_ieb_sources.py)
  {subject}/exam_guidelines/index.json    SAG source record (document itself
                                          fetched by fetch_ieb_sources.py)

Hand-edited IEB data (an ingested SAG, a curated syllabus overlay) must NOT
live in generated files — the generator would clobber it. Ingested documents
get their own files next to the index (e.g. exam_guidelines/sag_2026.md);
the generator never touches files it does not own.
...
PAST_PAPERS_PAGE = ("https://www.ieb.co.za/assessment/high-schools/"
                    "national-senior-certificate/nsc-past-papers")
DOCS_LIBRARY = "https://docs.ieb.co.za"
...
def derive_exam_guidelines_index(slug, display):
    return {
        "subject": display,
        "curriculum": "IEB",
        "document": f"IEB Subject Assessment Guidelines (SAG): {display}",
        "grades": "10-12 (one SAG per subject covers the FET phase)",
        "role": (
            "IEB counterpart of the DBE Grade 12 Examination Guidelines AND "
            "the DBE SBA programme: the single document that specifies how "
            "the IEB assesses this subject."),
        "source_page": SAG_PAGE,
        "status": "pending_fetch",
        "status_detail": (
            "ieb.co.za refuses automated fetching from this build "
            "environment (HTTP 403; the environment's egress policy also "
            "blocks general web fetches). URLs were located and recorded via "
            "web search on 2026-08-03 but the documents themselves are not "
            "yet fetched or verified. Run "
            "lessons/curriculum/IEB/scripts/fetch_ieb_sources.py from a "
            "network-enabled machine, then ingest per the CAPS method "
            "(full-text .md + structured .json next to this index)."),
        "terms_note": (
            "(c) IEB; not reproducible for commercial gain — see "
            "lessons/curriculum/IEB/SOURCES.md before ingesting."),
        "documents": [],
        "generated_by": GENERATED_BY,
    }


def derive_past_papers_index(slug, display):
    return {
        "subject": display,
        "grade": 12,
        "curriculum": "IEB",
        "portal": PAST_PAPERS_PAGE,
        "library": {
            "url": DOCS_LIBRARY,
            "access": (
                "Guest credentials published by the IEB itself on its FAQ "
                "(guest@ieb.co.za / guest) — intentionally provided public "
                "access, analogous to the DBE's public download links."),
            "folders": [
                "NSC Public Resources > NSC Examination Papers",
                "NSC Public Resources > NSC Marking Guidelines",
            ],
        },
        "portal_coverage": (
            "Previous 5 years of IEB NSC papers + marking guidelines; "
            "earlier years by email request to the IEB."),
        "grades_note": (
            "The IEB nationally examines Grade 12 (NSC) only. Grade 10-11 "
            "assessment at IEB schools is school-set per the SAGs, so no "
            "national Gr 10/11 papers exist to index (the CAPS tree's Gr 11 "
            "papers are DBE/provincial, a different examining body)."),
        "verified": None,
        "source_status": (
            "urls_recorded_not_verified: this build environment's network "
            "policy blocks ieb.co.za (proxy CONNECT 403, checked "
            "2026-08-03). Run lessons/curriculum/IEB/scripts/"
            "fetch_ieb_sources.py from a network-enabled machine to verify "
            "the portal and populate sessions[]."),
        "terms_note": (
            "(c) IEB; not reproducible for commercial gain - see "
            "lessons/curriculum/IEB/SOURCES.md before ingesting PDFs or "
            "embedding questions."),
        "sessions": [],
        "generated_by": GENERATED_BY,
    }
...
def build():
    ...
    for slug, display in SUBJECTS.items():
        ...
        # assessment-layer indexes
        out[base / "exam_guidelines" / "index.json"] = derive_exam_guidelines_index(slug, display)
        out[base / "past_papers" / "index.json"] = derive_past_papers_index(slug, display)
