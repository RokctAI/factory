...
Subcommands:
  ...
  fetch-papers
             ...
             {subject}/past_papers/index.json sessions[] where the subject
             is identifiable. The docs.ieb.co.za guest library (SharePoint)
             needs an interactive login; harvest it in a browser and
             `register` the files — this script does not automate
             credentialed access.
...
def cmd_fetch_papers(args):
    ...
    for slug, papers in sorted(per_subject.items()):
        def mutate(doc, papers=papers):
            ...
            doc["source_status"] = "portal_links_recorded"
        update_index(IEB_ROOT / slug / "past_papers" / "index.json", mutate)
        print(f"  [{slug}] {len(papers)} link(s) recorded")
    return 0
