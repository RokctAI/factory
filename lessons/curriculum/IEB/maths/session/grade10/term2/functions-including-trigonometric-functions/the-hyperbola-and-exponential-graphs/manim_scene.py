...
class HyperbolaExponentialSession(MovingCameraScene):
    ...
    def construct(self):
        ...
        # ============ Part 1 — Expert ============
        # --- Band 0 (subtopic_1): y = 8/x by table
        title = Tex(r"The Hyperbola: $y = \dfrac{8}{x}$").scale(1.25).to_edge(UP)
        self.play(Write(title))
        self.wait(2)
        b0_l1 = MathTex(r"x: 1,\; 2,\; 4,\; 8,\; 16 \;\to\; y: 8,\; 4,\; 2,\; 1,\; \tfrac{1}{2}").scale(1.05).shift(UP * 1.1)
        b0_l2 = Tex(r"As $x$ grows, $y$ sinks toward $0$ — never arrives").scale(1.0).shift(UP * 0.2)
        b0_l3 = MathTex(r"x = \tfrac{1}{2} \to y = 16, \quad x = \tfrac{1}{10} \to y = 80").scale(1.05).shift(DOWN * 0.7)
        b0_l4 = MathTex(r"x: -1,\; -2,\; -8 \;\to\; y: -8,\; -4,\; -1").scale(1.05).shift(DOWN * 1.6)
        ...
        self.wait(1.5)
        u = 0.30
        br1 = polyline([(1.2, 6.7), (2, 4), (3, 2.7), (4, 2), (6, 1.3), (7, 1.1)], org, u)
        br2 = polyline([(-7, -1.1), (-6, -1.3), (-4, -2), (-3, -2.7), (-2, -4), (-1.2, -6.7)], org, u)
        ...
        # --- Band 2 (subtopic_2): y = 8/x + 2 worked in full
        self.next_band(2)
        b2_title = Tex(r"Work $y = \dfrac{8}{x} + 2$ in full").scale(1.15).shift(band_shift(2) + UP * 2.2)
        ...
        self.wait(2.5)
        b2_l3 = MathTex(r"\frac{8}{x} = -2 \;\Rightarrow\; 8 = -2x \;\Rightarrow\; x = -4").scale(0.95).shift(band_shift(2) + DOWN * 0.7)
        self.play(Write(b2_l3))
        self.wait(2.5)
        b2_l4 = Tex(r"$x$-intercept $(-4; 0)$ — the shift created it").scale(1.05).shift(band_shift(2) + DOWN * 1.6)
        self.play(Write(b2_l4))
        self.play(Create(SurroundingRectangle(b2_l4, color=GREEN)))
        self.wait(2)
        b2_l5 = MathTex(r"x = 1 \to y = 10, \quad x = -2 \to y = -2").scale(1.05).shift(band_shift(2) + DOWN * 2.5)
        self.play(Write(b2_l5))
        self.wait(2.5)

        # --- Band 3 (subtopic_2): the sketch, and reading the equation off it
        self.next_band(3)
        b3_title = Tex(r"Sketch of $y = \dfrac{8}{x} + 2$").scale(1.15).shift(band_shift(3) + UP * 2.2)
        self.play(Write(b3_title))
        self.wait(1.5)
        org3 = band_shift(3) + LEFT * 4.2 + DOWN * 0.4
        ax3 = VGroup(Arrow(org3 + LEFT * 2.4, org3 + RIGHT * 2.4, buff=0),
                     Arrow(org3 + DOWN * 2.4, org3 + UP * 2.8, buff=0))
        u3 = 0.24
        asym3 = DashedLine(org3 + LEFT * 2.4 + UP * 2 * u3, org3 + RIGHT * 2.4 + UP * 2 * u3, color=BLUE)
        self.play(Create(ax3))
        self.play(Create(asym3), Write(MathTex(r"y = 2").scale(0.7).next_to(org3 + RIGHT * 2.4 + UP * 2 * u3, UP, buff=0.1)))
        self.wait(2)
        br3a = polyline([(0.8, 12), (1, 10), (2, 6), (4, 4), (6, 3.3), (8, 3)], org3, u3)
        br3b = polyline([(-8, 1), (-6, 0.7), (-4, 0), (-2, -2), (-1.5, -3.3), (-1, -6)], org3, u3)
        dInt = Dot(org3 + LEFT * 4 * u3, radius=0.06)
        self.play(Create(br3a))
        self.wait(2)
        self.play(Create(br3b), Create(dInt))
        self.wait(2)
        ...
        r3 = MathTex(r"(1; 10): \; 10 = \frac{a}{1} + 2 \Rightarrow a = 8").scale(1.0).shift(band_shift(3) + RIGHT * 3.4 + DOWN * 0.9)
        self.play(Write(r1))
        self.wait(2)
        self.play(Write(r2))
        self.wait(2)
        ...
        # --- Band 4 (subtopic_3): y = 3^x — the table and the features
        self.next_band(4)
        b4_title = Tex(r"The Exponential: $y = 3^x$").scale(1.2).shift(band_shift(4) + UP * 2.2)
        self.play(Write(b4_title))
        self.wait(1.5)
        b4_l1 = MathTex(r"x: -2, -1, 0, 1, 2, 3").scale(1.05).shift(band_shift(4) + UP * 1.2)
        b4_l2 = MathTex(r"y: \tfrac{1}{9}, \tfrac{1}{3}, 1, 3, 9, 27").scale(1.05).shift(band_shift(4) + UP * 0.3)
        ...
        u5 = 0.30
        growth = polyline([(-6, 0.01), (-2, 0.11), (-1, 0.33), (0, 1), (1, 3), (2, 9)], org5, u5)
        self.play(Create(growth), Create(Dot(org5 + UP * u5, radius=0.06)))
        self.wait(2)
        decay = polyline([(-2, 9), (-1, 3), (0, 1), (1, 0.33), (2, 0.11), (6, 0.01)], org5, u5, color=ORANGE)
        self.play(Create(decay))
        self.wait(2)
        g5a = Tex(r"$y = 3^x$: rises — GROWTH ($b > 1$)").scale(1.0).shift(band_shift(5) + RIGHT * 3.4 + UP * 1.0)
        g5b = Tex(r"$y = \left(\tfrac{1}{3}\right)^x$: falls — DECAY ($0 < b < 1$)").scale(1.0).shift(band_shift(5) + RIGHT * 3.4 + UP * 0.0)
        ...
        # --- Band 6 (subtopic_4): full sketch of y = 3^x - 9
        self.next_band(6)
        b6_title = Tex(r"Sketch $y = 3^x - 9$").scale(1.2).shift(band_shift(6) + UP * 2.2)
        self.play(Write(b6_title))
        self.wait(1.5)
        s1 = Tex(r"Asymptote: $y = -9$").scale(1.0).shift(band_shift(6) + RIGHT * 3.4 + UP * 1.3)
        s2 = MathTex(r"x = 0: \; 3^0 - 9 = -8 \;\to\; (0; -8)").scale(0.95).shift(band_shift(6) + RIGHT * 3.4 + UP * 0.4)
        s3 = MathTex(r"y = 0: \; 3^x = 9 \Rightarrow x = 2 \;\to\; (2; 0)").scale(0.95).shift(band_shift(6) + RIGHT * 3.4 + DOWN * 0.5)
        s4 = Tex(r"Range: $y > -9$").scale(1.0).shift(band_shift(6) + RIGHT * 3.4 + DOWN * 1.4)
        org6 = band_shift(6) + LEFT * 4.0 + UP * 0.4
        u6 = 0.26
        ax6 = VGroup(Arrow(org6 + LEFT * 2.4, org6 + RIGHT * 2.4, buff=0),
                     Arrow(org6 + DOWN * 2.6, org6 + UP * 2.2, buff=0))
        asym6 = DashedLine(org6 + LEFT * 2.4 + DOWN * 9 * u6, org6 + RIGHT * 2.4 + DOWN * 9 * u6, color=BLUE)
        ...
        self.play(Write(s3))
        self.wait(2.5)
        curve6 = polyline([(-5, -8.99), (-3, -8.96), (-1, -8.67), (0, -8), (1, -6), (2, 0), (2.5, 6.6)], org6, u6)
        self.play(Create(curve6), Create(Dot(org6 + DOWN * 8 * u6, radius=0.06)), Create(Dot(org6 + RIGHT * 2 * u6, radius=0.06)))
        self.wait(2)
        self.play(Write(s4))
        self.play(Create(SurroundingRectangle(s4, color=GREEN)))
        self.wait(2.5)

        # --- Band 7 (subtopic_4): negative a — y = -3^x + 3
        self.next_band(7)
        b7_title = Tex(r"Sketch $y = -3^x + 3$").scale(1.2).shift(band_shift(7) + UP * 2.2)
        self.play(Write(b7_title))
        self.wait(1.5)
        b7_l1 = Tex(r"Asymptote: $y = 3$; $a < 0$ reflects the curve down").scale(1.0).shift(band_shift(7) + UP * 1.2)
        b7_l2 = MathTex(r"x = 0: \; -1 + 3 = 2 \;\to\; (0; 2)").scale(1.05).shift(band_shift(7) + UP * 0.3)
        b7_l3 = MathTex(r"y = 0: \; 3^x = 3 \Rightarrow x = 1 \;\to\; (1; 0)").scale(1.05).shift(band_shift(7) + DOWN * 0.6)
        b7_l4 = Tex(r"Range: $y < 3$").scale(1.05).shift(band_shift(7) + DOWN * 1.5)
        ...
        self.wait(2)
        b7_l5 = Tex(r"In from the left just under $y = 3$, through $(0; 2)$,").scale(0.95).shift(band_shift(7) + DOWN * 2.3)
        b7_l6 = Tex(r"crosses at $(1; 0)$, then dives steeply").scale(0.95).shift(band_shift(7) + DOWN * 2.9)
        ...
        b8_l4 = Tex(r"4. Confusing $3^x$ (never turns) with $x^2$ (turns)").scale(1.0).shift(band_shift(8) + DOWN * 1.5)
        ...
        self.next_band(9)
        b9_title = Tex("Sharing a fixed pot: R240 for the pitch").scale(1.15).shift(band_shift(9) + UP * 2.2)
        self.play(Write(b9_title))
        self.wait(2)
        b9_l1 = MathTex(r"2 \to 120, \;\; 3 \to 80, \;\; 4 \to 60, \;\; 6 \to 40, \;\; 12 \to 20").scale(1.0).shift(band_shift(9) + UP * 1.1)
        self.play(Write(b9_l1))
        self.wait(2.5)
        b9_l2 = Tex(r"More players, smaller share — fast, then gentle").scale(1.0).shift(band_shift(9) + UP * 0.2)
        ...
        b10_l4 = Tex(r"Zero rounds $\Rightarrow$ one person: crosses at $1$").scale(1.05).shift(band_shift(10) + DOWN * 1.6)
        ...
        self.wait(2)
        b11_l5 = Tex(r"Ground floor or upstairs? $x^2$ turns; $3^x$ never does").scale(0.95).shift(band_shift(11) + DOWN * 2.6)
