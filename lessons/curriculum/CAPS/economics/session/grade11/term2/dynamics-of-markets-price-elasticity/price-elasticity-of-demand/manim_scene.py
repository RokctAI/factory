...
class PriceElasticityOfDemandSession(MovingCameraScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        f1 = MathTex(r"PED = \frac{\%\ \Delta\ \text{quantity demanded}}{\%\ \Delta\ \text{price}}").scale(1.2).shift(UP * 0.9)
        self.play(Write(f1))
        self.play(Create(SurroundingRectangle(f1, color=GREEN)))
        self.wait(2.5)
        f2 = MathTex(r"\%\ \text{change} = \frac{\text{change}}{\text{original}} \times 100").scale(1.15).shift(DOWN * 0.8)
        self.play(Write(f2))
        self.wait(2)
        ...
        self.play(Write(g3))
        self.wait(2)
        g4 = MathTex(r"\tfrac{5}{20} = 0{,}25 \text{ plank}; \quad \tfrac{30}{20} = 1{,}5 \text{ rubber band}").scale(1.0).shift(band_shift(8) + DOWN * 2.3)
        self.play(Write(g4))
