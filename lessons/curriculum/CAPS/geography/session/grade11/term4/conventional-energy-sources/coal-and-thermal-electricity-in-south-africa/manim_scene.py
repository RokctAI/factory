...
class CoalThermalElectricitySession(MovingCameraScene):
    ...
    def construct(self):
        ...
        self.wait(1.5)
        b2_chain = Tex(r"chemical $\Rightarrow$ heat $\Rightarrow$ kinetic $\Rightarrow$ electrical").scale(1.0).shift(band_shift(2) + UP * 1.4)
        ...
        for name, pos in stages:
            ...
            boxes.append(box)
            self.wait(1.2)
        b2_f1 = Tex(r"powder burns at 1 400 $^\circ$C").scale(0.85).shift(band_shift(2) + DOWN * 0.9 + LEFT * 4.6)
        ...
        self.wait(2)
        b7_l2 = Tex(r"Fission heat $\Rightarrow$ steam $\Rightarrow$ thermal plant").scale(1.0).shift(band_shift(7) + UP * 0.5)
        ...
        self.wait(2)
        b8_l1 = Tex(r"coal $\Rightarrow$ steam $\Rightarrow$ spin $\Rightarrow$ spark").scale(1.15).shift(band_shift(8) + UP * 1.2)
