...
# The tree diagram and two-way table are drawn from Lines/Rectangles/Tex.
#
...
class SimpleAndCompoundEventsSession(MovingCameraScene):
    ...
    def construct(self):
        ...
        self.wait(1.5)
        b1_l1 = MathTex(r"P(\text{even on a die}) = \tfrac{3}{6} = \tfrac{1}{2}").scale(1.1).shift(band_shift(1) + UP * 1.1)
        b1_l2 = MathTex(r"P(\text{WIN on the spinner}) = \tfrac{1}{8} = 0{,}125 = 12{,}5\%").scale(1.0).shift(band_shift(1) + UP * 0.1)
        self.play(Write(b1_l1)); self.wait(2.5)
        self.play(Write(b1_l2)); self.wait(2.5)
        b1_l3 = MathTex(r"P(\text{lose}) = 1 - \tfrac{1}{8} = \tfrac{7}{8}").scale(1.1).shift(band_shift(1) + DOWN * 0.9)
        self.play(Write(b1_l3))
        self.play(Create(SurroundingRectangle(b1_l3, color=GREEN)))
        self.wait(2.5)
        b1_wrong = MathTex(r"P = 1{,}2 \quad \text{(off the scale — always wrong)}").scale(1.0).shift(band_shift(1) + DOWN * 1.9)
        ...
        self.wait(1.5)
        b2_l1 = Tex("Taxis have no symmetry to consult — so watch").scale(1.0).shift(band_shift(2) + UP * 1.1)
        self.play(Write(b2_l1)); self.wait(2.5)
        b2_l2 = MathTex(r"\text{Full on 12 of 40 mornings: } \tfrac{12}{40} = 0{,}3").scale(1.05).shift(band_shift(2) + UP * 0.1)
        ...
        self.wait(1.5)
        b3_l1 = Tex("7 heads in 10 tosses: Tuesday. 700 in 1\\,000: crooked.").scale(0.95).shift(band_shift(3) + UP * 1.1)
        self.play(Write(b3_l1)); self.wait(2.5)
        b3_l2 = Tex("More trials $\\Rightarrow$ the estimate settles toward the truth").scale(0.95).shift(band_shift(3) + UP * 0.1)
        self.play(Write(b3_l2)); self.wait(2.5)
        b3_l3 = MathTex(r"\text{200 tosses, 92 heads: theory } 0{,}5, \text{ watched } \tfrac{92}{200} = 0{,}46").scale(0.9).shift(band_shift(3) + DOWN * 0.9)
        self.play(Write(b3_l3)); self.wait(2.5)
        b3_l4 = Tex(r"``0,46 sits close to 0,5, so the coin looks fair''").scale(0.95).shift(band_shift(3) + DOWN * 1.9)
        ...
        self.next_band(4)
        b4_title = Tex("Two tosses: the tree catches every route").scale(1.05).shift(band_shift(4) + UP * 2.6)
        ...
        self.wait(2.5)
        b4_l1 = MathTex(r"P(2\text{ heads}) = \tfrac{1}{4}; \;\; P(\text{exactly one}) = \tfrac{2}{4} = \tfrac{1}{2}").scale(0.95).shift(band_shift(4) + DOWN * 2.1)
        self.play(Write(b4_l1)); self.wait(2.5)
        b4_l2 = MathTex(r"P(\text{at least one head}) = 1 - \tfrac{1}{4} = \tfrac{3}{4}").scale(0.95).shift(band_shift(4) + DOWN * 2.9)
        ...
        self.next_band(5)
        b5_title = Tex(r"Unequal branches: pies run out one day in five").scale(1.0).shift(band_shift(5) + UP * 2.2)
        self.play(Write(b5_title))
        self.wait(1.5)
        b5_l1 = MathTex(r"\text{Pies both days: } 0{,}8 \times 0{,}8 = 0{,}64").scale(1.05).shift(band_shift(5) + UP * 1.1)
        b5_l2 = MathTex(r"\text{No pies both days: } 0{,}2 \times 0{,}2 = 0{,}04").scale(1.05).shift(band_shift(5) + UP * 0.2)
        b5_l3 = MathTex(r"\text{One yes, one no: } 1 - 0{,}64 - 0{,}04 = 0{,}32").scale(1.05).shift(band_shift(5) + DOWN * 0.7)
        self.play(Write(b5_l1)); self.wait(2.5)
        self.play(Write(b5_l2)); self.wait(2.5)
        self.play(Write(b5_l3)); self.wait(2.5)
        b5_l4 = MathTex(r"0{,}64 + 0{,}04 + 0{,}32 = 1 \;\checkmark").scale(1.05).shift(band_shift(5) + DOWN * 1.7)
        ...
        self.next_band(6)
        b6_title = Tex("120 visitors, two facts each").scale(1.1).shift(band_shift(6) + UP * 2.7)
        ...
        head = Tex(r"\quad Taxi \quad Foot \quad Total").scale(0.95).shift(band_shift(6) + UP * 1.2 + RIGHT * 1.3)
        self.play(Write(head))
        r1 = Tex(r"Learners: \; 28 \quad 42 \quad 70").scale(0.95).shift(band_shift(6) + UP * 0.0)
        r2 = Tex(r"Adults: \; 20 \quad 30 \quad 50").scale(0.95).shift(band_shift(6) + DOWN * 0.8)
        r3 = Tex(r"Total: \; 48 \quad 72 \quad 120").scale(0.95).shift(band_shift(6) + DOWN * 1.6)
        ...
        self.wait(1.5)
        b7_l1 = MathTex(r"P(\text{learner}) = \tfrac{70}{120} = \tfrac{7}{12} \approx 0{,}58").scale(1.05).shift(band_shift(7) + UP * 1.1)
        b7_l2 = MathTex(r"P(\text{adult AND taxi}) = \tfrac{20}{120} = \tfrac{1}{6}").scale(1.05).shift(band_shift(7) + UP * 0.1)
        self.play(Write(b7_l1)); self.wait(2.5)
        self.play(Write(b7_l2)); self.wait(2.5)
        b7_l3 = MathTex(r"\text{GIVEN a learner: } \tfrac{28}{70} = 0{,}4").scale(1.1).shift(band_shift(7) + DOWN * 0.9)
        ...
        b8_l2 = MathTex(r"\text{Red sweet blind: } \tfrac{3}{12} = \tfrac{1}{4}").scale(1.05).shift(band_shift(8) + UP * 0.2)
        self.play(Write(b8_l2))
        self.play(Create(SurroundingRectangle(b8_l2, color=GREEN)))
        self.wait(3)
        b8_l3 = Tex(r"WIN $\tfrac{1}{8}$ means LOSE $\tfrac{7}{8}$ — together exactly 1").scale(0.95).shift(band_shift(8) + DOWN * 0.8)
        self.play(Write(b8_l3)); self.wait(3)
        b8_l4 = Tex("A probability of 1,2 is not a big chance — it is broken").scale(0.95).shift(band_shift(8) + DOWN * 1.8)
        self.play(Write(b8_l4)); self.wait(3.5)

        # --- Band 9 (subtopic_6): forty mornings at the taxi ---
        self.next_band(9)
        b9_title = Tex("The taxi you watched for forty mornings").scale(1.1).shift(band_shift(9) + UP * 2.2)
        self.play(Write(b9_title))
        self.wait(2.5)
        b9_l1 = MathTex(r"\text{Full 12 of 40: } \tfrac{12}{40} = 0{,}3").scale(1.1).shift(band_shift(9) + UP * 1.2)
        ...
        b9_l3 = Tex("Forty mornings: a decent watch. Four: a rumour.").scale(1.0).shift(band_shift(9) + DOWN * 0.8)
        self.play(Write(b9_l3)); self.wait(3)
        b9_l4 = Tex(r"Theory 0,5, watching 0,46 — close, so the coin is fair").scale(0.95).shift(band_shift(9) + DOWN * 1.8)
        self.play(Write(b9_l4)); self.wait(3.5)

        # --- Band 10 (subtopic_7): branches and grids ---
        self.next_band(10)
        b10_title = Tex("Paths for sequences, cells for crowds").scale(1.1).shift(band_shift(10) + UP * 2.2)
        self.play(Write(b10_title))
        self.wait(2.5)
        b10_l1 = Tex(r"Multiply along the path: $\tfrac{1}{2} \times \tfrac{1}{2} = \tfrac{1}{4}$ each route").scale(0.95).shift(band_shift(10) + UP * 1.2)
        self.play(Write(b10_l1)); self.wait(3)
        b10_l2 = Tex("Exactly one head: TWO paths — the tree shows the one you forgot").scale(0.85).shift(band_shift(10) + UP * 0.2)
        self.play(Write(b10_l2)); self.wait(3)
        b10_l3 = MathTex(r"\text{Grid: } P(\text{learner}) = \tfrac{70}{120}; \;\; \text{given a learner: } \tfrac{28}{70}").scale(0.95).shift(band_shift(10) + DOWN * 0.8)
