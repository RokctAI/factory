...
class CreditSalesDebtorsJournalSession(MovingCameraScene):
    ...
    def construct(self):
        ...
        b2_l4 = Tex("DEBIT Cost of Sales R3 600;").scale(1.0).shift(band_shift(2) + DOWN * 1.1)
        b2_l5 = Tex("CREDIT Trading Stock R3 600").scale(1.0).shift(band_shift(2) + DOWN * 1.9)
        self.play(Write(b2_l4)); self.play(Write(b2_l5)); self.wait(2)
        b2_l6 = MathTex(r"\text{Check: } 8\,100 = 8\,100\ \checkmark\ \text{— detail daily, totals monthly}").scale(0.9).shift(band_shift(2) + DOWN * 2.9)
