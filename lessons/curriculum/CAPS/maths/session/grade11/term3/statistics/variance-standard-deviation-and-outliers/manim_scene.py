...
class VarianceStdDevOutliersSession(MovingCameraScene):
    ...
    def construct(self):
        ...
        self.wait(1.5)
        b4_l1 = MathTex(r"\text{Mean} = \tfrac{560}{10} = 56, \; \text{median} = 54").scale(1.1).shift(band_shift(4) + UP * 1.1)
        self.play(Write(b4_l1))
        self.wait(2.5)
        b4_l2 = MathTex(r"\text{mean} > \text{median}: \text{ positively skewed}").scale(1.05).shift(band_shift(4) + UP * 0.1)
        ...
        self.wait(2.5)
        b7_l2 = MathTex(r"\text{Without: } \bar{x} = 52{,}9, \sigma = 4{,}7").scale(1.1).shift(band_shift(7) + UP * 0.1)
